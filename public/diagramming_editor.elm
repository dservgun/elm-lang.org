...
Todo:
* Handle back space and delete in text box
* Handle keyboard control: characters being read when key is not down
* Handle tab to move to the next shape in the history
* Create title shape to be displayed
* Create connectors to shapes
* Classify diagrams, for example, the types needed could be
  ** Menu builder to build a runtime database aware menu
  ** A hierarchy builder to build hierarchies
  ** A flow chart to manage application flow
* Add a save button to save the diagram
* Create a websockets snap plugin to save the diagram and related information
* Create an autosave for non empty lists.
* ...--}
